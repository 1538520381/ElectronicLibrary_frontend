<template>
  <div class="image-viewer-container">
    ...
    <div class="image-viewer-wrapper">
      <div class="image-viewer" @wheel="handleWheel" @mousedown="startDrag" @mousemove="onDrag" @mouseup="stopDrag"
           @mouseleave="stopDrag" @dragstart.prevent>
        <img
            ref="viewerImage"
            :src="imageUrl"
            :style="imageStyle"
...
export default {
  ...
  methods: {
    ...
    handleWheel(event) {
      ...
      // 限制最小缩放比例为1（即图片宽度等于容器宽度）
      this.scale = Math.max(1, Math.min(3, newScale));
    },
    startDrag(event) {
    ...
    goToPage() {
      ...
      if (page > 0 && page <= this.book.page) {
        ...
        this.$message.warning(`页码超出范围，请输入1到${this.book.page}之间的数字`);
      }
    },
